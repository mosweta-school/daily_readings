...
export default function Readings() {
  ...
  // ── WebSocket: listens for admin toggle in real time ──────────────────────
  const connectWebSocket = () => {
    try {
      // Readings.jsx and Admin.jsx - production URL
const ws = new WebSocket("wss://dailyreadingsbackend.onrender.com");

      ws.onmessage = (event) => {
        const msg = JSON.parse(event.data);
        if (msg.type === "image_config") {
          setShowImage(msg.showImage);
        }
      };

      ws.onerror = () => {
        // WS unavailable — fall back to REST
        fetchImageConfig();
      };
  ...
  // ── REST fallback: fetch image config once on load ────────────────────────
  const fetchImageConfig = async () => {
    try {
      const res = await fetch(`${API}/api/image-config`);
      const data = await res.json();
      setShowImage(data.showImage);
    } catch {
      // If this also fails, default to showing the image
      setShowImage(true);
    }
  };
  ...
  const load = async (date) => {
    ...
    setError(false);
    try {
      const res = await fetch(`${API}/api/liturgy?date=${date}`);
  ...
  // Image renders only if admin has it enabled AND a URL actually exists
  const canShowImage = showImage && !!data.saint.image;
      ...
      <section style={styles.saintCard}>
        ...
        {canShowImage && (
          <img
            ...
            style={styles.saintImg}
            alt={`Portrait of ${data.saint.name}`}
            loading="lazy"
